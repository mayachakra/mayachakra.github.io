const inputBox = document.getElementById("text-box")
const taskTitle = document.getElementById("task-title")
const checkbox = document.getElementById("task-priority")

function addTask(){
   
    if(inputBox.value === ''){

        alert("You must enter a task!")
    }
    else{
        let li = document.createElement("li");
        li.innerHTML = inputBox.value;
        taskTitle.appendChild(li);
        let span = document.createElement("span");
        span.innerHTML = "\u00d7";
        li.appendChild(span);
        let checkbox = document.createElement("check");
        checkbox.innerHTML = taskTitle.value;
        checkbox.appendChild(check);
    }
    inputBox.value = "";
    saveData();

}

taskTitle.addEventListener("click", function(e){
    if(e.target.tagName === "LI"){
        e.target.classList.toggle("checked");
        saveData();


    }
    else if(e.target.tagName === "SPAN"){

        e.target.parentElement.remove();
        saveData();

    }
}, false)
   
function saveData(){

    localStorage.setItem("data", taskTitle.innerHTML);
}
function displayTask(){
    taskTitle.innerHTML = localStorage.getItem("data");
}


